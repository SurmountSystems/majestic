...
explicitly. It does not ingest a random JSON array as story cards. Story-card JSON and `characters/{id}.toml` need an explicit path plus `--service` and `--account`. The scan walks
...
still wins on explicit paths. Session JSONL, story-card JSON arrays, and character TOML still need those flags or `-o`. Home scan does not treat a random JSON array as story cards. Do
